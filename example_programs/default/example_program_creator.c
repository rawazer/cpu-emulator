 ...
 */
uint8_t ROM[] = {
    0x04, 0x00, 0x00, // LD R0, RAM[0] (load data from RAM[0] to R0)
    0x04, 0x01, 0x01, // LD R1, RAM[1] (load data from RAM[1] to R1)
    0x04, 0x02, 0x02, // LD R2, RAM[2] (load data from RAM[2] to R2)
    0x04, 0x03, 0x01, // LD R3, RAM[1] (load data from RAM[1] to R3)
    0x02, 0x10, // ADD R1, R0 (add R0 to R1)
    0x03, 0x02, // SUB R0, R2 (subtract R2 from R0)
    0x08, 0x03, // CMP R0, R3 (compare R0 and R3)
    0x07, 0x16, // JZ 0x12 (jump to 0x12 if ZF is 1)
    0x06, 0x0C, // JMP 0x08 (jump to 0x08)
    0xFF // HALT
};
uint8_t RAM[] = {
    0x05,
